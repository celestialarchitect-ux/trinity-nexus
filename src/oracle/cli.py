...
def cli(ctx: click.Context, thread: str):
    ...
    if ctx.invoked_subcommand is None:
        from oracle.repl import run_repl
...
def ask(prompt, thread: str, stream: bool):
    ...
    try:
        if stream:
            from oracle.repl import _stream_answer
...
def mcp():
    ...
    from oracle.mcp_server import run_stdio
...
def ingest(directory: str):
    ...
    from oracle.retrieval import ingest_directory
...
def reflect(n_turns: int, apply: bool, remember: bool):
    ...
    from oracle.reflect import reflect as _reflect
...
def mesh_keygen(label: str | None):
    ...
    from oracle.mesh import new_identity
...
def mesh_id():
    ...
    from oracle.mesh import load_identity
...
def mesh_add_peer(pubkey_b64: str, url: str, label: str):
    ...
    from oracle.mesh.identity import add_peer
...
def mesh_peers():
    ...
    from oracle.mesh.identity import load_allowlist
...
def mesh_export(origins: str):
    ...
    from oracle.mesh import build_bundle
...
def mesh_push(peer_url: str, origins: str):
    ...
    from oracle.mesh import push_bundle
...
def mesh_pull(peer_url: str):
    ...
    from oracle.mesh import pull_bundle
...
def evolve(intent, min_score: float, force: bool):
    ...
    from oracle.skills.evolve import evolve_from_router_gap, evolve_skill
...
def distill(lookback_hours: int, dry_run: bool, train: bool):
    ...
    from oracle.distillation import DistillationOrchestrator
