...
Exposes:
  Tools:
    - nexus_ask            run a full agentic turn on the local Oracle
    - nexus_retrieve       semantic search over the user's ingested corpus
    - nexus_recall         semantic search over archival memory
    - nexus_remember       store a fact in archival memory
    - nexus_skill_list     enumerate the skill library
    - nexus_skill_run      run a specific skill by id
  Resources:
    - nexus://core-memory  the core memory markdown block
    - nexus://skills       a JSON listing of all skills
"""
...
def build_server() -> FastMCP:
    mcp = FastMCP("trinity-nexus")

    # ----------------------------- Tools -----------------------------

    @mcp.tool()
    def nexus_ask(prompt: str, thread_id: str = "mcp-default") -> str:
    ...
    @mcp.tool()
    def nexus_retrieve(query: str, k: int = 5) -> list[dict]:
    ...
    @mcp.tool()
    def nexus_recall(query: str, k: int = 5) -> list[dict]:
    ...
    @mcp.tool()
    def nexus_remember(fact: str, tags: list[str] | None = None) -> dict:
    ...
    @mcp.tool()
    def nexus_skill_list() -> list[dict]:
    ...
    @mcp.tool()
    def nexus_skill_run(skill_id: str, inputs_json: str) -> dict:
        """Execute a specific Oracle skill by id.

        Args:
            skill_id: one of the ids from nexus_skill_list
    ...
    @mcp.resource("nexus://core-memory")
    def core_memory() -> str:
